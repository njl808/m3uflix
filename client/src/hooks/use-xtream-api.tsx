...
export function useAuthentication(api: XtreamAPI | null) {
  return useQuery({
    queryKey: ['/api/auth', api ? 'connected' : 'disconnected'],
...
export function useLiveStreams(api: XtreamAPI | null, categoryId?: string) {
  return useQuery({
    queryKey: ['/api/live-streams', api ? 'connected' : 'disconnected', categoryId ?? 'all'],
...
export function useVODStreams(api: XtreamAPI | null, categoryId?: string) {
  return useQuery({
    queryKey: ['/api/vod-streams', api ? 'connected' : 'disconnected', categoryId ?? 'all'],
...
export function useSeries(api: XtreamAPI | null, categoryId?: string) {
  return useQuery({
    queryKey: ['/api/series', api ? 'connected' : 'disconnected', categoryId ?? 'all'],
...
export function useCategories(api: XtreamAPI | null, type: 'live' | 'vod' | 'series') {
  return useQuery({
    queryKey: ['/api/categories', api ? 'connected' : 'disconnected', type],
    queryFn: () => {
...
export function useEPG(api: XtreamAPI | null, streamId: number | null) {
  return useQuery({
    queryKey: ['/api/epg', api ? 'connected' : 'disconnected', streamId],
